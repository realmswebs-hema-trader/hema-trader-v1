...
const AdminRoute = ({ children }: { children: ReactNode }) => {
  ...
  const hasAdminRole = profile?.roles?.includes('admin') || profile?.isAdmin === true;
  const isAdminEmail = user?.email === 'realmswebs@gmail.com';
  
  return (hasAdminRole || isAdminEmail) ? <>{children}</> : <Navigate to="/" />;
};
...
function AppRoutes() {
  ...
  if (loading) return (
    <div className="flex h-[80vh] flex-col items-center justify-center gap-4 text-slate-500">
      <Loader2 className="h-8 w-8 animate-spin text-amber-500" />
      <span className="text-[10px] font-bold uppercase tracking-wider text-white">Loading marketplace...</span>
    </div>
  );
